...
function updateWizardUI() {
    ...
    const stepNames = ['Welcome', 'Storage', 'Profile', 'Assembler', 'Memories', 'Finish'];
...
function showTab(name, clickedBtn) {
    ...
    if (name === 'sources') { loadScheduler(); loadFolders(); loadWebhooks(); }
...
async function loadDashboard() {
    ...
    try {
        ...
        // Scheduler stats for dashboard cards
        try {
...
async function runSchedulerNow() {
    ...
    try {
        ...
        const d = await res.json();
        const all = Object.entries(d.folders || {}).map(([n, r]) => `${n}: +${r.added}`);
...
function openGlobalSearch() {
    ...
    document.getElementById('global-search-results').innerHTML =
        '<div class="global-search-hint">Type to search memories, templates, folders...</div>';
...
async function globalSearch() {
    ...
    const el = document.getElementById('global-search-results');
    if (!q) {
        el.innerHTML = '<div class="global-search-hint">Type to search memories, templates, folders...</div>';
        gsItems = [];
        gsSelectedIndex = -1;
        return;
    }
...
function globalSearchSelect(type, label) {
    ...
    else if (type === 'folder') { showTab('sources', null); }
}
